module Input.Float exposing (Options, StringOptions, input, inputString, defaultOptions, defaultStringOptions)
        ...
        filterKey =
            (\event ->
                if event.ctrlKey || event.altKey || event.metaKey then
                    ...
                    (isMinus event.keyCode
                        || isDot event.keyCode
                        || isComma event.keyCode
                    )
                then
                    Json.fail "minus or dot or comma"
                else if
